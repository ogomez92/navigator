...
impl Selection {
    pub fn clear(&mut self) { self.items.clear(); self.anchor = None; self.focus = None; }

    pub fn contains(&self, i: usize) -> bool { self.items.contains(&i) }
    pub fn len(&self) -> usize { self.items.len() }
    pub fn is_empty(&self) -> bool { self.items.is_empty() }
    pub fn iter(&self) -> impl Iterator<Item = usize> + '_ { self.items.iter().copied() }

    pub fn focus(&self) -> Option<usize> { self.focus }
    pub fn set_focus(&mut self, i: Option<usize>) { self.focus = i; }
    pub fn anchor(&self) -> Option<usize> { self.anchor }
    pub fn set_anchor(&mut self, i: Option<usize>) { self.anchor = i; }
    ...
    pub fn toggle(&mut self, i: usize) {
        if !self.items.insert(i) { self.items.remove(&i); }
        self.focus = Some(i);
        if self.anchor.is_none() { self.anchor = Some(i); }
    }

    /// Idempotent add. Used when mirroring LVN_ITEMCHANGED — unlike
    /// `toggle`, this doesn't flip off an already-present item.
    pub fn insert(&mut self, i: usize) { self.items.insert(i); }

    /// Idempotent remove.
    pub fn remove(&mut self, i: usize) { self.items.remove(&i); }
    ...
    pub fn extend_to(&mut self, i: usize) {
        ...
        for k in lo..=hi { self.items.insert(k); }
        self.focus = Some(i);
    }
}
