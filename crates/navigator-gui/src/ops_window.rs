...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    SW_SHOW, WINDOW_EX_STYLE, WM_CLOSE, WM_COMMAND, WM_DESTROY, WM_SETFONT, WM_SIZE,
...
use crate::clipboard::{entry_details, entry_label, load_history, HistoryEntry};
...
pub fn open(parent: HWND, state: Arc<AppState>) {
    let hwnd = match ensure_window(parent, state) {
        Ok(h) => h,
        Err(e) => { tracing::warn!("ops-history window failed: {e:?}"); return; }
    };
...
fn build_children(parent: HWND, state: Arc<AppState>) -> Data {
    ...
    apply_font(list); apply_font(detail); apply_font(btn_restore); apply_font(btn_close);
    ...
    Data { state, list, detail, btn_restore, btn_close, entries: Vec::new() }
}
...
fn layout(hwnd: HWND) {
    ...
    unsafe {
        ...
        let _ = MoveWindow(d.detail,      pad,  detail_y, (w - pad * 2).max(40), detail_h, true);
...
fn refresh_entries(hwnd: HWND) {
    let Some(d) = (unsafe { data(hwnd) }) else { return; };
    let prev = unsafe {
        SendMessageW(d.list, LB_GETCURSEL, Some(WPARAM(0)), Some(LPARAM(0))).0 as i32
    };

    unsafe { SendMessageW(d.list, LB_RESETCONTENT, Some(WPARAM(0)), Some(LPARAM(0))); }
    d.entries = load_history();
    if d.entries.is_empty() {
        let w: Vec<u16> = "(no recent operations)\0".encode_utf16().collect();
        unsafe {
            SendMessageW(d.list, LB_ADDSTRING, Some(WPARAM(0)), Some(LPARAM(w.as_ptr() as isize)));
        }
    ...
    let sel = if prev >= 0 && (prev as usize) < d.entries.len() { prev as usize } else { 0 };
    unsafe {
