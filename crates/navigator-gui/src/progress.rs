...
use windows::Win32::UI::WindowsAndMessaging::{
    BS_PUSHBUTTON, CW_USEDEFAULT, CreateWindowExW, DefWindowProcW,
    GetWindowLongPtrW, HCURSOR, HMENU, IDC_ARROW, LoadCursorW, RegisterClassExW, SendMessageW,
    SetWindowLongPtrW, SetWindowTextW, ShowWindow, SW_HIDE, SW_SHOW, WINDOW_EX_STYLE, WM_APP,
    WM_COMMAND, WM_DESTROY, WM_SETFONT, WNDCLASSEXW, WS_BORDER, WS_CAPTION, WS_CHILD,
    WS_OVERLAPPED, WS_SYSMENU, WS_TABSTOP, WS_VISIBLE, GWLP_USERDATA, CS_HREDRAW, CS_VREDRAW,
};
...
impl ProgressHandle {
    pub fn post_status(&self, current: &str, done: u64, total: u64) {
        ...
        unsafe {
            let _ = windows::Win32::UI::WindowsAndMessaging::PostMessageW(
                Some(self.hwnd), WMAPP_PROGRESS_STATUS, WPARAM(0), LPARAM(payload as isize),
            );
        }
    }
    ...
    pub fn post_log(&self, line: &str) {
        ...
        unsafe {
            let _ = windows::Win32::UI::WindowsAndMessaging::PostMessageW(
                Some(self.hwnd), WMAPP_PROGRESS_LOG, WPARAM(0), LPARAM(payload as isize),
            );
        }
    }
    ...
    pub fn post_done(&self, success: bool) {
        unsafe {
            let _ = windows::Win32::UI::WindowsAndMessaging::PostMessageW(
                Some(self.hwnd), WMAPP_PROGRESS_DONE,
                WPARAM(if success { 1 } else { 0 }), LPARAM(0),
            );
        }
    }
...
pub fn open(parent: HWND) -> windows::core::Result<ProgressHandle> {
    ...
    if let Some(h) = SINGLETON.get().and_then(|m| m.lock().unwrap().clone()) {
        if unsafe { windows::Win32::UI::WindowsAndMessaging::IsWindow(Some(h.hwnd)) }.as_bool() {
            unsafe { let _ = ShowWindow(h.hwnd, SW_SHOW); }
...
fn bring_to_foreground(hwnd: HWND) {
    ...
    unsafe {
        ...
        let target = if let Some(d) = data(hwnd) { d.log } else { hwnd };
...
unsafe extern "system" fn wnd_proc(hwnd: HWND, msg: u32, wp: WPARAM, lp: LPARAM) -> LRESULT {
    match msg {
        WMAPP_PROGRESS_STATUS => unsafe {
            let Some(d) = data(hwnd) else { return LRESULT(0) };
        ...
        WMAPP_PROGRESS_DONE => unsafe {
            let Some(d) = data(hwnd) else { return LRESULT(0) };
            d.finished = true;
            set_text(d.label_current, if wp.0 == 1 { "Done." } else { "Finished with errors." });
        ...
        WM_COMMAND => unsafe {
            ...
            match cmd {
                IDC_BTN_CANCEL => {
                    if let Some(f) = d.cancel_flag.lock().unwrap().as_mut() { f(); }
                    set_text(d.label_current, "Cancelling…");
                    let _ = windows::Win32::UI::Input::KeyboardAndMouse::EnableWindow(d.btn_cancel, false);
                }
                IDC_BTN_CLOSE => { let _ = ShowWindow(hwnd, SW_HIDE); }
                _ => {}
            }
            LRESULT(0)
        },
...
fn append_log(hwnd: HWND, line: &str) {
    ...
    unsafe {
        ...
        SendMessageW(hwnd, 0x00C2, Some(WPARAM(0)), Some(LPARAM(w.as_ptr() as isize)));
        SendMessageW(hwnd, 0x00B7, Some(WPARAM(0)), Some(LPARAM(0)));
    }
}

fn fmt_bytes(n: u64) -> String { crate::listview::format_size(n) }
...
fn mkmulti(parent: HWND, x: i32, y: i32, w: i32, h: i32) -> HWND {
    let style = WS_CHILD.0 | WS_VISIBLE.0 | WS_BORDER.0 | WS_VSCROLL
        | ES_MULTILINE | ES_READONLY | ES_AUTOVSCROLL | WS_TABSTOP.0;
    unsafe {
        CreateWindowExW(
            WINDOW_EX_STYLE(0),
            w!("EDIT"),
