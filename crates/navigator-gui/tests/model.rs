...
fn entry_full(name: &str, kind: EntryKind, size: u64, modified: u64, created: u64) -> Entry {
    Entry {
        name: name.into(),
        kind,
        size,
...
fn file(name: &str) -> Entry {
    entry(name, EntryKind::File, false, false)
}
fn dir(name: &str) -> Entry {
    entry(name, EntryKind::Directory, false, false)
}

fn cwd() -> NavPath {
    NavPath::new(r"C:\ignored").unwrap()
}
...
fn find_prefix_matches_first_occurrence() {
    ...
    m.set_listing(
        cwd(),
        vec![dir("Options"), dir("OneDrive"), file("readme.md")],
    );
...
fn sort_by_type_puts_extensionless_first() {
    ...
    m.set_listing(
        cwd(),
        vec![file("README"), file("notes.txt"), file("Makefile")],
    );
    let names: Vec<_> = (0..m.len()).map(|i| m.get(i).unwrap().name).collect();
...
fn sort_by_type_descending_reverses_within_kind() {
    ...
    m.set_listing(cwd(), vec![file("a.txt"), file("b.md"), file("c.txt")]);
    let names: Vec<_> = (0..m.len()).map(|i| m.get(i).unwrap().name).collect();
