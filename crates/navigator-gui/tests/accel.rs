...
fn chord_with_no_key_yields_none() {
    let c = ShortcutChord { ctrl: true, ..Default::default() };
...
fn chord_always_sets_fvirtkey() {
    ...
    let c = ShortcutChord { key: "T".into(), ..Default::default() };
...
fn default_terminal_chord_parses() {
    let c = ShortcutChord {
        alt: true, key: "T".into(), ..Default::default()
    };
    let r = chord_to_accel(&c);
    assert!(r.is_some(), "the default Open-in-Terminal chord must be bindable");
}
