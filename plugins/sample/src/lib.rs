...
use navigator_plugin_api::abi::{HostApi, Plugin, PluginInfo, Str, ABI_VERSION};
...
pub unsafe extern "C" fn navigator_plugin_entry(api: *const HostApi, out: *mut Plugin) -> i32 {
    if api.is_null() || out.is_null() { return 1; }
    let api_ref = unsafe { &*api };
    if api_ref.abi_version != ABI_VERSION { return 2; }

    let state = Box::new(State { host: api, nav_count: Mutex::new(0) });
...
unsafe extern "C" fn on_navigated(plugin: *mut c_void, path: Str) {
    ...
    *count += 1;
    ...
    if !state.host.is_null() {
        ...
        unsafe { (host.speak)(host.host, Str::from(&msg), false); }
    }
}
