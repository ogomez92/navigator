...
- **Both steps are gated on the message's root window being the main window** (`GetAncestor(msg.hwnd, GA_ROOT) == hwnd`). `TranslateAcceleratorW` does not check which window a key message is for — it matches the chord and sends `WM_COMMAND` to the main window regardless — so a key typed into a modeless tool window (viewer, progress, space breakdown) fired the main binding instead: F5 in the breakdown tree refreshed the listing rather than rescanning, Ctrl+C in the viewer ran Copy on the listview selection. Those windows handle their own keys through subclasses (`install_tab_nav`, `install_esc_close`, and their own procs); the pump must leave their messages alone.
...
**A selected folder is a sweep root.** `split_extract_selection` (pure, takes `(path, is_dir)` straight off the model so it costs no `stat`) routes files to a direct extract and directories to `sweep_archives`, which walks the tree and extracts everything it finds *in place* — each archive next to itself, wherever it was found. Three rules, each with a test:

- **The sweep sees `SWEEP_EXTENSIONS`, not `EXTRACTABLE_EXTENSIONS`.** The full list is deliberately wide — 7-Zip opens `.exe` (SFX), `.jar`, `.msi`, `.apk`, `.iso` — and that width is only safe for a file the user pointed at. Swept recursively with the default `delete_when_extracted`, it would "extract" every binary in a source tree and then delete it. `sweep_set_is_a_strict_subset_of_the_extractable_set` pins both halves.
- **The walk runs on `navigator-extract-survey`**, never the pump: it's unbounded IO. It posts `WMAPP_EXTRACT_SURVEYED` with `(targets, 7z path, body)`; the UI thread runs the confirm and spawns the extractor. Same shape as the empty-trash survey, for the same reason.
- **A sweep always confirms first, defaulting to No.** One keystroke on a folder row can reach a whole drive, and the purge is a plain `remove_file` — no `.trash`, no undo. A flat selection of archives keeps the old zero-dialog path. `sweep_confirm_body` is pure so the wording and the 12-item truncation are testable.

**The purge deletes the volume set, not the file 7z was handed.** `purge_targets` does one `read_dir` and feeds the pure `volume_set`, which collects `movie.partN.rar` siblings (or old-style `movie.rNN` next to `movie.rar`). Deleting `movie.part1.rar` alone leaves eight parts that are now unopenable — the same as not purging at all. The sweep correspondingly skips continuation volumes (`is_continuation_volume`): 7-Zip pulls the whole set in from part 1, so queueing part 2 is one guaranteed failure per part. `archive_stem` drops a trailing `.partN` so the wrapper folder isn't named after one volume of the set.

**A numbered split set is recognised by filename, not by extension.** `movie.7z.001` has extension `001`, which is in no extension table and never will be — `split_volume` parses `<base>.<3+ digits>` and `is_extractable` asks it *before* consulting `EXTRACTABLE_EXTENSIONS`. The base keeps its own extension because it names the file the parts join back into, which is what the rest of the rules key off: `is_sweepable` takes the set only when that base is itself sweepable (`foo.7z.001` yes, `foo.mkv.001` no — a recursive walk must not join a split media file and then purge the parts), `archive_stem` falls through to `movie`, `is_continuation_volume` skips `.002`+, and `volume_set` purges every numbered sibling.

**Selecting a whole volume set extracts once, not once per part.** `drop_covered_volumes` (inside `split_extract_selection`, pure) removes any continuation whose *first* part is also selected — Ctrl+A over a folder holding `movie.7z.001`…`.008` is the normal gesture, and 7-Zip pulls the parts in from the first, so the other seven are guaranteed failures the user then has to interpret. A continuation selected *alone* is still attempted, so the error names the file they picked rather than the selection coming back as "no extractable archives selected". `volume_key` tags each scheme (`rar:` / `split:`) so a `movie.part1.rar` and a `movie.002` sharing a folder can't be read as parts of each other.
...
**Extract holds an `OpGuard` and its children die with us.** It is a long-running file operation like any other, so `spawn_extract` counts it in `ops_in_flight` — without that, Alt+F4 closed the window mid-extract with no warning at all. And both 7z calls go through `run_child`, which registers the child with the process-wide kill-on-close job (`navigator_rclone::kill_child_with_process`, exported from the rclone driver so there is one job for the process, not two). `Command::status()` hands back no `Child` to register, and an unregistered 7z outlives us: Windows doesn't kill children with their parent, so closing mid-extract left an invisible `7z.exe` unpacking into a folder nobody was watching — and then *not* purging, because the worker that owns that half of the job had died with the window. `run_zip` uses the same helper.
...
Not swept, on purpose: per-volume `.trash/`, per-volume `.navigator_backup/`, and `<exe_dir>/.remote-cache/`. All hold user data, and none lives in `%TEMP%`.
...
### Filename encoding: rclone's namespace is not the filesystem's

**A path on an rclone command line, and a name in a `--files-from` list, are not OS filenames.** The local backend keeps names in an internal *standard* form and runs them through an [encoder](https://rclone.org/overview/#restricted-filenames) on the way to and from the disk: Windows forbids `" * : < > ? \ |`, so each maps to its full-width twin (`|` ⇄ `｜`, U+FF5C), and a full-width character that was *already* in the name is escaped with a quote rune `‛` (U+201B) to keep the two apart. Navigator lists directories itself via `FindFirstFileExW`, so every name it holds is in OS form, and handing one to rclone unconverted fails **in two different ways, one of them silent**:

- On argv (`copyto` / `moveto` / `purge` / …) rclone decodes the path, finds nothing, and reports `directory not found`.
- In a `--files-from` list the entry is matched against rclone's *own* listing of the source root, which is in standard form. A raw OS name matches nothing, so rclone transfers nothing, logs nothing and **exits 0** — the paste reports success and the files never moved.

`navigator-rclone/src/encoding.rs` is the translation (`to_standard_name` for one component, `to_standard_path` per component of a local path). `path_arg` and `batch::TempList::write` are the two call sites; remote paths go through `rclone_arg()` and must **never** be translated, since remote backends carry their own encoding. Only the decode direction (OS → standard) is needed, and no rule touches ASCII — which is what keeps a drive letter's colon and the path separators intact.

- **This replaced a global `--local-encoding None`, which was wrong in both directions.** It only ever addressed the argv half, and it makes a name containing a literal `‛` permanently unreachable: under `None` rclone still doubles the rune when it lists (`a‛b` reads back as `a‛‛b`) but no longer halves it when it opens the file, so every path built from that listing points at a file that does not exist. That is precisely the shape yt-dlp output takes on after one mixed-encoding copy (`‛＂Title‛＂ [id].mp3`), and it is how a folder of them became unmovable while the paste kept claiming success. Don't reintroduce the flag.
- **The rules are measured, not derived.** `tests/encoding.rs` re-runs the differential against whatever rclone is on PATH: every ordered pair over the encoder's alphabet, the same alphabet in trailing position, the real yt-dlp names, and an end-to-end `--files-from` batch that pins the silent-loss behaviour itself (untranslated: exit 0, one file of three). If rclone ever changes, that suite fails rather than the paste.
- **The corners that are not guessable**: `／` (U+FF0F) is never decoded even though `Slash` is in the encoding, because it would inject a separator — and a `‛` in front of it is therefore kept, not consumed. Same for `␀`..`␟` (U+2400..=U+241F). `␡` (U+2421) *is* decodable, so a literal one gains an escape. `␠` / `．` decode only in the last position of a component, which is the only place the encoder puts them. A lone `‛` doubles; a `‛‛` stays — so `a‛b` and `a‛‛b` share one standard name and rclone genuinely cannot tell them apart (`rclone cat` reads the same file for both). That ambiguity is rclone's, and a batch that hits it surfaces through `run_group`'s missing-destination check rather than silently copying the wrong file.
- **Translated names must not reach the user.** rclone answers a batched dry-run in the namespace it was asked in, so `resolve_conflicts` maps each reported name back through the group's own `names` before it reaches a dialog — being asked to confirm overwriting `"quoted".mp3` when the row on screen reads `＂quoted＂.mp3` is a question about an unidentifiable file. It's an exact lookup, not a reverse encoder.
...
**A background operation owns its files, not the user's location.** An op can run for minutes; the user is free to walk away and do something else while it does, and finishing gives it no claim on where they are when it lands.

- **`WorkerCtx.refresh_target` is the folder captured at spawn, and it is only ever re-listed through `refresh_with_focus`, which checks `AppState::is_viewing` first.** It used to post that listing unconditionally, so a paste started in `A` dragged the user out of whatever folder they had since browsed to — the window simply changed folders on its own, mid-task. `AppState::refresh_dir(dir)` is the same rule for the workers that don't hold a `WorkerCtx` (both shell-op reapers, remote purge, remote upload); a detached shell copy can outlive the folder by hours.
- **"Where the user is" is `nav_target`, not `model.cwd()`.** A navigation is a *queued scan*, so for the whole gap between the keypress and the listing arriving, `cwd()` still names the folder being left. A worker that consulted `cwd()` in that window concluded the user hadn't moved and queued a re-list of the old folder — and since the scan worker is FIFO, that listing landed *after* the one the user asked for and silently pulled them back. `navigate` publishes the destination before queueing; `settle_nav_target` retires it when the listing (or `WMAPP_DIR_ERROR`) lands, and only when the path matches, so the older of two queued navigations can't cancel the newer one's claim. `effective_folder` is the pure rule, unit-tested.
- **`pending_focus` is scoped to the listing it was meant for** via `focus_target_belongs` — a target belongs to exactly one listing, the one for its parent (drive roots belong to This PC). It is armed *before* the op runs — `op_delete` picks the row that will survive the delete, the paste worker names the file it created — so an arrival in an unrelated folder would otherwise consume it and either match a coincidentally same-named row or drop the caret to row 0.
- **A re-listing of the folder already on screen keeps the caret where it was.** `WMAPP_DIR_LISTED` reads `Model::focused_name` *before* installing (indices don't survive a re-list; the name does) and `refocus_after_up` restores it when nothing armed a target. Row 0 is right for arriving somewhere new and wrong for F5, a filter/sort toggle, or an operation finishing under you — a refresh that moves the caret is indistinguishable from the app navigating on its own.
- Consequences worth keeping: `rescan` deliberately skips the navigation ceremony (no history entry, no navigation cue, no type-ahead reset) because re-reading the folder you're standing in is a redraw, not a move — the same reasoning that keeps `navigate` silent when the target equals the cwd.
...
### Backup / restore (Ctrl+Shift+B / Ctrl+Shift+R)

`backup.rs` owns the pure/persistence half; `op_backup` / `op_restore_backup` + the two `WorkerCtx` runners in `app.rs` are the impure drivers; `backup_dialog.rs` is the picker. Built for the game-save loop: back up a folder once, restore it as many times as the game eats it.

- **Backup copies the focused entry to `<volume_root>\.navigator_backup\<id>\<basename>`** — same-volume staging like `.trash`, but a *copy* (`rclone copyto`), because the original stays in place. `<id>` is `<unix_ts>_<pid>_<counter>`; the pid matters here where trash tolerates its absence, because the id is a recorded name a later restore resolves. Records live in `<exe_dir>/backups.json`, newest first, written by the worker **only after the copy succeeds** — the file must never offer a restore whose data isn't there. A failed or cancelled backup also deletes its half-written directory: unrecorded, nothing would ever point at it again.
- **A backup is only taken where a restore is guaranteed.** `backup_dir_on_volume_of` returns `None` for UNC (same server-root-litter problem as trash), remote and sentinel paths — the guard lives in the naming function, not just `op_backup`, for the same reason `trash_dir_on_volume_of` carries its own UNC check. `backup_dir_is_never_named_on_unc_remote_or_sentinel_paths` pins it. Remote entries can't be backed up at all: the restore's rename-to-trash half has no remote equivalent, and half a restore is data loss.
- **Restore replaces, but never destroys silently.** Two invocations, strictly ordered: the current version (if any) is renamed into `.trash` first, and the backup is copied back **only if that rename succeeded**. The picker dialog is the confirmation (no second prompt — asking twice trains Enter-through); what makes that safe is the trash staging. The backup itself is never consumed. No undo entry is pushed: undo-restore would race the restored copy at the original path, and the pre-restore state is already reachable in `.trash` — the spoken summary says so. A cancel after the rename reports "previous version is in trash", not "failed" — the truthful mid-state.
- **The picker is scoped to the current folder, never the whole record file.** `backups_in_dir` keeps only the records whose `original` is a direct child of the cwd (`dir_key` folds case and separators, so `D:/code/tools/` and `d:\CODE\Tools` scope alike), and `preselect_index` then opens on the focused entry's newest backup — stand-on-it → Ctrl+Shift+R → Enter restores what you're looking at. An unscoped list put *every* recorded backup one mis-keyed Enter away from any folder: Ctrl+Shift+R in `D:\code\tools` could overwrite `C:\meow`, and with a single record on file it did so with **no dialog at all**. Scoping by folder rather than by the focused row is deliberate — the case this exists for is a game that just ate its save, so the item itself is often gone and there is no row left to stand on, but its parent folder is still where the user is. The dialog is skipped only when the single candidate *is* the focused row; a lone backup of a sibling still asks, because otherwise the thing restored is not the thing under the caret. An empty scope announces how many backups exist elsewhere, so "none here" can't be heard as "your backup is gone". The folder is named in the dialog caption (which a screen reader announces on open) and in its heading, so rows carry only name/kind/date instead of repeating the same path on every line. A record whose backup dir has since been hand-deleted is pruned at pick time rather than left to fail forever.
- Like `.trash`, `.navigator_backup` is never auto-purged and never swept, and `backups.json` is deliberately uncapped — capping it would silently orphan directories that still hold data. Each record is two paths, so it never grows the way `clipboard_history.json` does; reads/writes happen on op workers.
...
### Space breakdown (Ctrl+Shift+S)

A `du`-style answer to "what is taking the space?": a `SysTreeView32` over the focused folder (or the current folder when a file / nothing is focused — Alt+L's rule; on This PC the focused *drive*), every folder carrying the total of everything beneath it, children largest first. `spacemap.rs` is the pure model (arena `SpaceTree`, unit-tested with no HWND); `space_window.rs` is the modeless singleton window (same shape as `viewer`); `AppState::op_space_breakdown` resolves the root, `scan_space` runs the walk on `navigator-space-scan` and posts `WMAPP_SPACE_SCANNED`, `show_space_breakdown` opens the window on the UI thread.

- **A native tree view, not a listview or a text dump.** Screen readers already read one — level, expanded/collapsed, "3 of 12" — and Left/Right/Backspace/Home/End/type-ahead come from the control. Enter (`NM_RETURN`, and double-click) goes there: a folder is navigated *into*, a file is `jump_to`'d in its parent, and the window closes because the scan is stale the moment the user starts changing things. Delete (`TVN_KEYDOWN`) and F5 are the only keys we add.
- **Delete runs `AppState::delete_targets`, never a fourth delete path.** That is `op_delete` minus the selection bookkeeping, split out for exactly this caller: remote → confirm + purge with no undo, UNC → the shell's own prompt, local → `.trash` with undo, whichever the row's path is. It returns whether anything *started*; only then is the row removed from the tree (`SpaceTree::remove` subtracts its size and counts from every ancestor, and the window relabels the ancestor chain and the siblings whose share changed). The one way the tree can overstate what is gone is a shell prompt on a share answered No — the shell says "delete cancelled" aloud, and F5 is the truth again. The root row refuses Delete: it is the folder being sized.
- **Two producers, one aggregator.** A remote root is one `rclone lsjson --recursive` (the only rclone call that returns every file's size in one round-trip — `lsjson` reports `-1` on directories and `rclone size` answers one folder per spawn) fed through `SpaceTree::from_items`. A local root is walked by `SpaceTree::scan_local_with` *straight into the arena*: `props::walk_tree` materialises a path string (twice — `TreeEntry` keeps a lower-cased key) per entry, and a million-entry drive is the case this screen exists for. Both go through the same `Builder`, so implied-parent completion, the roll-up and the largest-first ordering are written once; `local_scan_matches_the_flat_builder_for_the_same_tree` pins the two against each other.
- **Folder sizes are rolled up from files, never read.** A directory's reported `Size` (`-1` on most backends, `0` locally, a real number on a few) is discarded on entry — trusting it double-counts against the children we sum. Sizes are apparent byte lengths, not allocation; the heading says so because "size" on a folder screen is read as disk usage.
- **Rows are inserted on expand, and capped.** The control is not virtual; inserting a drive's million rows up front freezes the pump for the whole insert. `ensure_populated` fills a folder the first time it expands (root is populated explicitly *before* `TVM_EXPAND`, which does not reliably raise `TVN_ITEMEXPANDING`), and a folder with more than `MAX_ROWS_PER_FOLDER` (5000) children shows the largest plus one `OVERFLOW_ROW` naming how much the tail adds up to. That row's `lParam` is `-1`; `node_of` rejects it so Enter/Delete on it are a spoken no-op, never an index.
- **The scan is cancellable and reports as it goes.** `space_scan_cancel` holds an `Arc<AtomicBool>`; a new `scan_space` trips the previous one, and the window's `WM_DESTROY` calls `cancel_space_scan` so a rescan in flight can't re-open a window the user just closed. The local walker takes a `tick(files) -> bool` hook (once per directory) that the worker uses both to stop and to speak "scanning, N files so far" at `announce_interval_secs` — a drive scan runs for minutes and silence that long reads as "nothing happened". A remote listing can't be interrupted mid-call, so its result is checked against the flag on return and dropped.
- Sentinels: This PC needs a focused drive row (`drive_path_from_display`, the same trap `op_show_properties` documents); the remotes root is refused. A failed listing is *spoken* (`SpaceScanOutcome::Failed`), never rendered as an empty tree — zero bytes with no explanation reads as "this folder is empty".
...
- **The upload back writes `--inplace`, and that is the only way to keep a Unix remote's permissions.** rclone's default upload for any backend with `PartialUploads` (sftp, local) writes `<name>.<rand>.partial` and renames it over the target, so what comes back is a *different file*: on Linux, a fresh inode carrying umask-derived mode and the connecting account's ownership. Measured against a real sftp remote, a `600 root:root` file edited through this flow came back `644 root:root` with a new inode. There is nothing to restore afterwards either — rclone cannot read a Unix mode over sftp at all (`rclone backend features` reports `ReadMetadata: false`, `WriteMetadata: false`, `MetadataInfo: null`, and `lsjson --metadata` emits no `Metadata` key), and `lsf --format` has no permissions field, so `--metadata` / `--metadata-set` have nothing to work with. Preserving the *file* is the mechanism. `Operation::CopyToInPlace` is `copyto` plus that flag, and it is deliberately a separate variant from `CopyTo`: the cost is atomicity (a failed write truncates the destination instead of leaving it untouched), which is only acceptable here because the staged local file *is* the complete new contents, so a retry always finishes the job (see the failure rule below). Never reach for it in the paste or download path, where the source may be the only copy. `inplace_overwrites_the_same_file_and_the_default_replaces_it` pins both halves against a real rclone via hard links (Windows has no mode bits, but "still the same file object?" is the same question).
- **The upload prompt is a TaskDialog, purely so Yes can be focused.** It is normally raised while *another* app holds the foreground — the editor the user just saved in — and a dialog that is never activated never hands keyboard focus to its default control, so `nDefaultButton` alone left the caret on the frame: nothing announced, and a Tab needed before Enter did anything. `MessageBoxW` offers no hook to fix that; `TaskDialogIndirect` takes a callback, and `upload_prompt_callback` calls `SetFocus` on the Yes button at `TDN_CREATED`. That focus is recorded as the dialog's focused control, so it is what gets restored whenever the user finally alt-tabs back. The "never steal foreground" rule is unchanged: the callback re-asserts foreground only when navigator already held it, which is passed in through `lpCallbackData`.
- **A failed write-back raises a dialog with Retry, not just a spoken sentence.** Because the write is `--inplace`, a transfer that dies part-way leaves the remote file **truncated** rather than untouched — measured against a real sftp remote, killing an upload at 51% left 1638400 of 3000000 bytes, checksum matching neither the old nor the new version (mode and inode survived, as designed). The old failure path spoke one sentence and cleared the record, which is nowhere near enough warning: it lands while the user is still typing in the editor they saved from, and nothing survives it on screen. The worker now posts `WMAPP_REMOTE_UPLOAD_FAILED` and the UI thread offers Retry (focused and defaulted) / Not now. Retrying is always the fix — the staged local file holds the full contents, verified end-to-end: a retry after that 51% kill restored all 3000000 bytes with the checksum and the `640` mode intact. Three rules: `prompting` is deliberately **left set** by the worker so the watcher cannot race a fresh prompt against the retry, and only the dialog clears it (`report_upload_failure` clears it itself if there is no hwnd to post to, or the post fails — otherwise the file would silently never prompt again); `started` gates the truncation sentence, because rclone can also fail before opening the destination and crying damage every time trains the user to click past it; and the staged path is named in the body *and* logged at WARN, because after a declined retry that path is the only thing that can still recover the remote once the process exits (records are per-process and are not persisted).

### Config & persistence

- `ConfigHandle::load_or_default()` is infallible — a corrupt `config.toml` logs a warning and returns defaults.
- `config.toml`, `plugins/`, `clipboard.json`, `clipboard_history.json`, `backups.json`, and `.trash/` all live next to the exe (or in the case of `.trash` and `.navigator_backup`, at each volume root). `navigator_config::exe_dir()` is the source of truth; don't hardcode.
...
See `README.md` for the user-facing table. User-bound actions live under `shortcuts` in `config.toml`; `navigator_config::shortcuts::default_actions()` returns the seeded defaults (Copy/Cut/Paste/PasteSpecial[Ctrl+Shift+V]/Append/CopyPaths/SelectAll/Rename/Refresh/ToggleHidden/ToggleSystem/Search/NavigateUp/Hist Back+Forward/Undo + Hotspot1..10 + HotspotSet1..10 + ShowProperties[Alt+Enter] + DumpTree[Alt+L] + NewFolder[Ctrl+N] + NewFile[Ctrl+Shift+N] + Backup[Ctrl+Shift+B] + RestoreBackup[Ctrl+Shift+R] + SpaceBreakdown[Ctrl+Shift+S]). The accel table is rebuilt on startup and on shortcut-editor save via `window::rebuild_accels`. `default_chords_are_unique` in `navigator-config/tests/config.rs` guards against a new default silently shadowing an existing chord — the accel table matches modifiers strictly, so a collision means one action never fires.
