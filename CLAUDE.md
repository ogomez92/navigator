...
The **one** exception is Ctrl+Alt+V (paste from the OS clipboard), which hands the batch to the Windows shell copy engine on purpose — the shell is what antivirus recognises as a legitimate file operation, so a paste of thousands of files avoids the heuristics that flag rclone streaming them. See *Detached shell copy* below. Everything else, including every rclone-clipboard paste, stays on the rclone path.

### Detached shell copy (`shell_op.rs`)
